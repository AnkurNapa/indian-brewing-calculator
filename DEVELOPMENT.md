...
- All state lives in one `AppState` object (`src/hooks/useWaterProfile.ts`), persisted to `localStorage` under `indian-brewing-calculator/app-state/v1`.
  ...
  - **Golden rule: call each `useLocalStorageState`-backed hook (`useWaterProfile`, `useFermentationBatches`, `useRecipeSnapshots`) exactly once, in `page.tsx`, and pass the value/setter down as props.** These hooks read `localStorage` once on mount and never listen for writes from other instances -- calling the same hook a second time in a child component creates an independent, silently-stale copy. This was a real bug: `useFermentationBatches()` was called separately in `page.tsx`, `FermentationTrackerPanel`, and `BackupPanel`; page.tsx's copy (feeding Home) never remounts for the life of the session, so logging a batch in Fermentation Tracker left Home showing stale/empty data until a full page reload. Fixed by making `page.tsx` the sole caller and threading `batches`/`onBatchesChange` down as props, same as every other shared field.
